...
export const AuthStore = signalStore(
  ...
  withMethods(
    (
      authState,
      authService = inject(AuthGraphqlService),
      ...
      login: rxMethod<{ email: string; password: string }>(
        pipe(
          tap(() =>
            patchState(authState, { inProgress: true, error: undefined }),
          ),
              ...
              tapResponse({
                next: (user: AuthUser) => {
                  ...
                  const expirationDate = new Date(
                    new Date().getTime() + user.expiration!,
                  );
        ...
        pipe(
          ...
          switchMap((user: Partial<AuthUser>) => {
            return authService.signup(user).pipe(
              tapResponse({
                next: () =>
                  router.navigate([], { queryParams: { success: user.email } }),
                error: (error: string) =>
                  patchState(authState, { inProgress: false, error }),
                finalize: () => patchState(authState, { inProgress: false }),
              }),
