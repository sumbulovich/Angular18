...
export const PlacesStore = signalStore(
  ...
  withComputed((store, userPlacesStore = inject(UserPlacesStore)) => ({
    unselectedPlaces: computed(() =>
      store
        .placesEntities()
        ?.filter(
          (f) =>
            !userPlacesStore.userPlacesEntities().some((s) => s.id === f.id),
        ),
    ),
    areLoading: computed(
      () => store.isLoading() || userPlacesStore.isLoading(),
    ),
  })),
          ...
          return placesService.getPlaces().pipe(
            tapResponse({
              next: (places: Place[]) =>
                patchState(
                  store,
                  setAllEntities(places, { collection: 'places' }),
                ),
